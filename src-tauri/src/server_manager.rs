...
impl ServerManager {
                ...
                let exit_check = {
                    ...
                    if let Some(child) = p_map.get_mut(&id_mv) {
                        match child.try_wait() {
                            Ok(Some(status)) => {
                                println!("[ServerManager] Server {} exited: {:?}", id_mv, status);
                                p_map.remove(&id_mv);
                                Some(Ok(status))
                            }
                            Ok(None) => None,
                    ...
                    if let Ok(status) = res {
                        ...
                        if let Some(server) = s_map.get_mut(&id_mv) {
                            if server.status == ServerStatus::Stopping || server.status == ServerStatus::Stopped {
                                ...
                                // Already marked crashed
                            } else {
                                // Unexpected exit (crash) - non-zero exit = crashed
    ...
    async fn get_paper_url(&self, version: &str) -> Result<String> {
        ...
        let client = reqwest::Client::builder()
            .user_agent("MinecraftServerManager/0.1.0 (https://github.com/user/prismarine)")
            .build()?;
        let builds: serde_json::Value = client.get(&builds_url).send().await?.json().await?;

        let builds_arr = builds.as_array().context("No builds found")?;
        let latest_stable = builds_arr
            .iter()
            .find(|b| b["channel"].as_str() == Some("STABLE"))
            .or_else(|| builds_arr.first())
            .context("No builds found")?;

        let url = latest_stable["downloads"]["server:default"]["url"]
            .as_str()
    ...
    async fn get_velocity_url(&self, version: &str) -> Result<String> {
        ...
        let latest_stable = builds_arr
            .iter()
            .filter(|b| {
                b["channel"].as_str() == Some("RECOMMENDED")
                    || b["channel"].as_str() == Some("STABLE")
            })
            .last()
            .or_else(|| builds_arr.last())
            .context("No builds found")?;
    ...
    async fn get_waterfall_url(&self, version: &str) -> Result<String> {
        let builds_url = format!(
            "https://fill.papermc.io/v3/projects/waterfall/versions/{}/builds",
            version
        );
        let client = reqwest::Client::builder()
            .user_agent("MinecraftServerManager/0.1.0 (https://github.com/user/prismarine)")
    ...
    pub async fn fetch_paper_versions(&self) -> Result<Vec<String>> {
        ...
        let versions_map = resp["versions"]
            .as_object()
            .context("Invalid v3 API response format")?;

        let mut versions: Vec<String> = Vec::new();
        for (_group, arr) in versions_map {
    ...
    pub async fn fetch_velocity_versions(&self) -> Result<Vec<String>> {
        let url = "https://fill.papermc.io/v3/projects/velocity";
        let client = reqwest::Client::builder()
            .user_agent("MinecraftServerManager/0.1.0 (https://github.com/user/prismarine)")
            .build()?;
        let resp: serde_json::Value = client.get(url).send().await?.json().await?;
    ...
    pub async fn fetch_waterfall_versions(&self) -> Result<Vec<String>> {
        let url = "https://fill.papermc.io/v3/projects/waterfall";
        let client = reqwest::Client::builder()
            .user_agent("MinecraftServerManager/0.1.0 (https://github.com/user/prismarine)")
            .build()?;
        let resp: serde_json::Value = client.get(url).send().await?.json().await?;
